...
document.addEventListener('DOMContentLoaded', function() {
  const customSelects = document.querySelectorAll('.projects-navigation__item.custom-select');
...
class PriceRange {
  ...
  initEvents() {
    ...
    this.minThumb.addEventListener('touchstart', (e) => this.startDrag('min', e));
    this.maxThumb.addEventListener('touchstart', (e) => this.startDrag('max', e));
  ...
  getValueFromPosition(position) {
    const value = this.minValue + (this.maxValue - this.minValue) * (position / 100);
...
function init() {
    ...
    map.behaviors.disable([
        'scrollZoom', 'dblClickZoom', 
        'multiTouchZoom', 'rightMouseButtonMagnifier'
    ]);
